...
public class SessionPanel extends JPanel {
	...
	private final HintCheckBox testCors;
			private final JLabel headerToRemoveLabel = new JLabel("要移除的请求头");
	private final PlaceholderTextArea headersToRemoveText = new PlaceholderTextArea(3, textFieldWidth);
			private final JLabel restrictToScopeLabel = new JLabel("限制范围");
	...
	public SessionPanel(String sessionName, MainPanel mainPanel) {
		...
		JLabel headerToReplaceLabel = new JLabel("要替换的请求头");
		c.gridy = 0;
		...
		removeHeaders = new HintCheckBox("移除请求头", false, "定义的请求头将被移除");
		c.gridwidth = 1;
		c.insets = new Insets(5, 0, 0, 20);
		c.gridy++;
		sessionPanel.add(removeHeaders, c);
		
		filterRequestsWithSameHeader = new JCheckBox("过滤具有相同请求头的请求", false);
		c.gridx = 1;
		//sessionPanel.add(filterRequestsWithSameHeader, c);
		restrictToScope = new HintCheckBox("限制范围", false, "会话仅在定义的范围内重复");
		c.gridx = 2;
		sessionPanel.add(restrictToScope, c);
		
		//testCors = new JCheckBox("Test CORS", false);
		testCors = new HintCheckBox("测试CORS", false, "HTTP方法将设置为OPTIONS");
		c.gridx = 3;
		...
		infoButton.addActionListener(e -> {
			...
			try {
				...
				JOptionPane.showMessageDialog(this, "无法打开浏览器。", "错误", JOptionPane.WARNING_MESSAGE);
			}
		});
	...
	public void updateMatchAndReplaceButtonText() {
		if(matchAndReplaceList.size() > 0) {
			matchAndReplaceButton.setText("匹配和替换 (" + matchAndReplaceList.size() + ")");
		}
		else {
			matchAndReplaceButton.setText("匹配和替换");
		}
	}
