...
public class DataExportDialog {
	...
	public DataExportDialog(CenterPanel centerPanel) {
		...
		inputPanel.add(new JLabel("选择导出格式。"));
    	JRadioButton htmlReport = new JRadioButton("HTML导出", true);
    	JRadioButton xmlReport = new JRadioButton("XML导出");
    	ButtonGroup group = new ButtonGroup();
    	group.add(htmlReport);
    	group.add(xmlReport);
    	inputPanel.add(htmlReport);
    	inputPanel.add(xmlReport);
    	JCheckBox doBase64Encode = new JCheckBox("对请求和响应进行Base64编码", true);
    	...
    	inputPanel.add(new JLabel("选择要包含在导出中的列。"));
		...
		int result = JOptionPane.showConfirmDialog(centerPanel, inputPanel, "导出表格数据",
